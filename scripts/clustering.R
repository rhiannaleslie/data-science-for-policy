...
p <- ggplot(elbow_df) +
  ...
  theme_bw() +
  labs(x = "number of clusters", y = "wss", title = "CPI all sectors elbow plot") 

ggsave("outputs/sectors_elbow.png", height = 10,  width = 12)

#running the clustering algorithm
cpi_sectors_clustered <- get_cluster_partitions(data = cpi_sectors_wider, group_col = "sector", cluster_num = 3L)

cpi_sectors_preds <- cpi_sectors_clustered$data %>%
  dplyr::mutate(partition = paste0("cluster ", partition))

#visualising output
clustering_visualisation(cluster_data = cpi_sectors_preds, group_col = "sector", lab = "Sectors")

#evaluate clustering - we use intrinsic measurements here
cat(paste0("The silhoutte score for the CPI all sector clustering is: ", cpi_sectors_clustered$score), file = "outputs/clustering_sil_scores.txt")
...
ggplot(elbow_df) +
  ...
  labs(x = "number of clusters", y = "wss", title = "CPI food products elbow plot") 

ggsave("outputs/food_type_elbow.png", height = 10, width = 12)

#running the clustering algorithm
cpi_food_clustered <- get_cluster_partitions(data = cpi_food_wider, group_col = "food_type", cluster_num = 4L)
...
#visualising output
clustering_visualisation(cluster_data = cpi_food_preds, group_col = "food_type", lab = "Food type")

#evaluate clustering - we use intrinsic measurements here
cat(paste0("\n\nThe silhoutte score for the CPI food type clustering is: ", cpi_food_clustered$score), file = "outputs/clustering_sil_scores.txt", append = TRUE)
